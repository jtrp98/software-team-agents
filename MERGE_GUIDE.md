...
| `software-team-agents/contracts/*.yaml` (9 files) | Copy verbatim to `contracts/`. These are the machine-readable contracts for the nine agents — the orchestrator resolves them at `<project-root>/contracts/<agent-name>.yaml` and `npm run orchestrate -- --check-contracts` verifies them against its own registry, so they must land at the target's root, not inside `orchestrator/`. | If the target already has a `contracts/` folder for something else, copy these in alongside rather than replacing anything: the filenames are agent names (`backend-engineer.yaml` and so on) and a collision means the target has a file named after one of these agents. Flag that to the user instead of overwriting. |
