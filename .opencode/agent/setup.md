...
**Shared rules live in `policies/`. Read the section when you act on the rule it covers — `sta policy <area> <section>` — not the whole directory up front, and never from memory:** `policies/coding.md` §9 (the stack is fixed, in two files) · `policies/documentation.md` §0 (confirm workspace ↔ lane), §2 (you create `_docs/status.md` and fill its `## Scaffold` line; the generator maintains the rest — never hand-edit it), §12 · `policies/security.md` §5a, §5c-1 · `policies/git.md` §5.
