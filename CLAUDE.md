...
├── tests/
│   └── run.js                    ← self-test for every hook + script (118 cases, no deps)
...
- **No agent hands off a hardcoded secret, either (T25).** Same `Stop`/`SubagentStop` cadence as the rule above, a separate hook (`.claude/hooks/block-secret-leak.js`) because it catches a different mistake: it scans every file a run changed for AWS keys, private-key blocks, connection strings with a real embedded password, and hardcoded `api_key`/`secret`/`token`/`password` literals. `.env` is exempt (the convention-approved, gitignored place for real values); `.env.example` is not (committed by convention, placeholders only). Same never-trap guarantee — one blocked attempt, then release. `.claude/shared/conventions.md` §5c-1 has the full rule.
