...
| `business-analyst` | business requirements | `qa`, `design`, `requirement` (amend) | `requirement.md` |
| `system-analyst` | feasibility + data model | `requirement`, `qa`, stack files | `design.md` |
...
| `backend-engineer` | API/DB code | `plan`, `design`, `requirement`, `test-plan`, `review`, `qa` | app code |
| `frontend-engineer` | UI code | same, plus the module's signed UX artifact | app code |
| `reviewer` | independent code review; never fixes | `requirement`, `design`, `plan`, `test-plan`, real code | `review.md`, `review/phase-N.md` |
| `qa-engineer` | verification | all docs + `schema.prisma` + real code | `qa.md`, `qa/phase-N.md`, `plan.md` Status cells, add-only 🔒 gates |
| `security` | security audit | `requirement`, `design`, `qa`, `schema.prisma`, real code | `security.md` |
| `devops` | deploy, CI, migrations | `status`, `qa`, `security`, `plan`, `design`, `schema.prisma`, stack files | `deploy.md`, infra files |
...
Who owns which file is the Roles table's Writes column. `design.md`, `qa.md` and `status.md` each
...
P3 established no winning or losing category, changing no route. Automatic benchmark-driven bypass remains rejected; any future adjustment stays a smaller existing-mechanism `workflows/*.yml`.
