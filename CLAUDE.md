...
- **Nothing reaches a FULL QA round without the full deterministic sweep.** The QA evidence package carries the run's `deterministic_gate` instruction: enabled rounds consume its structured result without re-running checks; disabled rounds receive the complete mechanical sweep instruction and execute it before verifying. A skipped or wholly unconfigured sweep is never treated as a pass.
