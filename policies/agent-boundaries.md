...
**The two reviewers are part of the chain, not exceptions to it.** In orchestrated runs, `workflows/*.yml` closes every code-changing pipeline with `qa-engineer`, and `security` joins whenever a sensitive area or the data model is touched — verification is what the automation is *for*, so it is chained like any other stage rather than left for a caller to remember. What is never automated is acting on a human-only risk or verdict: those points are listed below. Outside a pipeline, both remain invokable by name exactly as before ("ตรวจงานหน่อย", "security review", ฯลฯ), and everything about how they run once invoked (their own FULL/TARGETED gating, their own escalation rules) is unchanged.

**The following human-only gates stop in both modes when their stated trigger is present — autonomous mode does not remove them, it only drives the pipeline up to them:**

1. **`business-analyst` on a material unresolved business choice or missing confirming authority.** Complete intake with a source, confirming owner, bounded scope, stable REQ/AC IDs and explicit decision status may be normalized and validated without asking the same questions again. Missing confirmation evidence keeps the interactive interview fallback. A material unresolved business choice or missing authority is never inferred or waived by a generic approval: the run stops with the exact question and owner. Code-answerable questions are preserved for `system-analyst` evidence instead of being sent to a person as business choices.
...
**Within a phase, always run `backend-engineer` to completion first, then `frontend-engineer`.** This applies in both manual and autonomous mode — it isn't a human gate (§6 above), because it isn't a decision at all, it's an ordering rule like any other in this file: the pipeline (or the user) simply invokes them in that order instead of together.
