...
**External services don't widen this boundary.** MCP servers are reached only through frozen, fail-closed allowlists — Figma read-only (five inspect tools) and Claude Design (`orchestrator/src/integration/claudeDesignMcp.ts`: 9 read tools; only `copy_files`/`create_project`/`write_files` in explicit write mode, all destructive/publishing/membership tools refused). What a uxui-designer puts on a design canvas is a **draft**, still subject to human sign-off like every other knowledge-lane output. Any future code-intelligence provider (graphify, ADR-006) is optional, default-ON only when its index is fresh (ADR-006 Option A; index build/refresh itself stays consent-gated, never silent): it must never become a dependency of a stage, never decide a verdict, and its cache lives outside every repo.
