...
const root = process.env.AGENTCLAUDE_WORKSPACE_ROOT || process.cwd();
...
function toolName(input) {
  for (const value of [input.tool_name, input.toolName, input.tool, input.name]) {
...
function toolParameters(input) {
  const holders = [input.tool_info, input.toolInfo, input];
