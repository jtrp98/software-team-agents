...
Every agent also reads `_docs/status.md` when it starts and regenerates it (`node .Codex/scripts/generate-status.js` — T51, `policies/documentation.md` §2) when it finishes, rather than hand-editing it — that's left out of the table above rather than repeated on all ten rows.
