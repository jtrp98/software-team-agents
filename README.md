...
ต้นทุนที่แลกมาคือขั้นตอนที่เยอะขึ้น `policies/agent-boundaries.md` §8 (Right-sizing) มีไว้แก้ปัญหานี้โดยเฉพาะ เพื่อให้งานเล็กๆ ไม่ต้องจ่ายราคาทั้ง pipeline — ดูรายละเอียดด้านล่าง
...
└── module/
    └── sales-crm/
        ...
        ├── plan.md               ← project-manager  (task table, Status cell + เพิ่มธง security: qa-engineer, T52)
...
.claude/
├── shared/
│   ├── conventions.md            ← ตอนนี้เหลือแค่ pointer สั้นๆ ไปที่ policies/ (T49 ย้ายกติกาไปแล้ว)
...
├── src/
│   ├── runtime/runtimeAdapter.ts    ← interface กลาง (T108) — framework เรียก agent ผ่านนี้ ไม่ผูก runtime ใดตัวหนึ่ง
│   ├── runtime/runtimeExecutor.ts   ← AgentExecutor ที่ประกอบ prompt/อ่าน artifact กลับ แล้วส่งผ่าน RuntimeAdapter (T108)
│   ├── runtime/claudeCodeAdapter.ts ← RuntimeAdapter ของ Claude Code (T109) — เรียก `claude -p --agent <role>` จริง ต่อกับ `.claude/agents/<role>.md`
...
- **`check-schema-contract.js`** — `qa-engineer` รันผ่าน Bash เพื่อเทียบ `schema.prisma` กับ Data Model ใน `design.md` ของทุก module แบบ field-by-field พร้อมเช็คว่า model ที่ไม่มีใครประกาศเป็นการด้นสด (§7 ใน `policies/architecture.md`)
- **`check-status-sync.js`** — เทียบ `status.md` กับจำนวนแถว `verified` จริงใน `plan.md`'s task table (T52) ของทุก module หา `implemented`/`verified` ที่เขียนไว้ผิดจากของจริง (เป็น second opinion อิสระ ตั้งแต่ T51 ไม่มี agent ตัวไหนเขียน `status.md` เองแล้ว)
- **`generate-status.js`** — สร้าง `_docs/status.md` ใหม่ทั้งไฟล์จาก `plan.md`/`review.md`/`security.md`/`deploy.md` จริง (T51) — ทุก agent รันตัวนี้แทนการแก้ `status.md` เอง
