...
| **Antigravity** | 🧪 **Experimental** — runtime id `antigravity`, binary `agy`; `sta run --runtime antigravity` และ `dev\|ba --runtime antigravity` รับแล้ว. Spike บน agy 1.1.24/Windows 11 พิสูจน์ headless `-p` + `--output-format json`, `--model` (refuse จริงเมื่อ model ไม่รู้จัก), `--effort`, `--conversation`, `--print-timeout`. ไม่มี project agent store → role ถูก fold เข้า prompt; envelope ไม่มีช่อง cost → ไม่ claim `COST_REPORTING`; หา hook dispatch ไม่เจอเลย → Target-write stages ถูกปฏิเสธ ไม่ใช่วิ่งแบบ unguarded | **unguarded** — `.agents/hooks.json` + `.agents/hooks/sta-guard.js` ถูก generate และ fail closed โดยโครงสร้าง แต่ spike ทำให้ AGY `PreToolUse` ยิงไม่สำเร็จเลยใน 5 คอนฟิก → ถือว่า guard ทุกตัว inactive จนกว่า deny path จะถูก demonstrate จริง (T-V6-013) |
...
`Claude:`/`Codex:`/`OpenCode:`). `status` ไม่เขียนอะไรเลย ปลอดภัยรันซ้ำได้ทุกเมื่อ.
