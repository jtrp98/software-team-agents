...
- **Each agent writes only what its contract gives it.** `contracts/<role>.yaml` carries `write`,
  ...
  `STA_ROLE` (set by the orchestrator). An interactive session sets no such variable and
...
**Target visibility:** the read direction above also runs the other way, symmetrically and optionally. The Knowledge workspace resolves registered Targets through `targets.yaml` plus this machine's `.workflow/targets.local.yaml` mapping; when Targets resolve, `software-team-agents open` sets `STA_TARGET_WORK_ROOTS` (every mapped Target, all `access: "read"`) for read-only context. Unset or unresolved, analysis work continues exactly as before — Targets stay optional, nothing about the analysis workflow depends on them. `system-analyst` is the one agent that reads them today: amending a module that's already implemented, with a Target resolvable, it reads the real schema off the Target using the Target-resolved stack metadata rather than a hardcoded path before treating a change as additive/breaking, and reports drift against `design.md` plainly instead of trusting `design.md`'s memory of what got built. No write channel opens either direction — this is read-only, the same posture `STA_KNOWLEDGE_ROOT` gives orchestrated engineer stages.

Every agent also reads `_docs/status.md` when it starts and regenerates it (`node .claude/scripts/generate-status.js`, `policies/documentation.md` §2) when it finishes, rather than hand-editing it — that's left out of the table above rather than repeated on all eleven rows. The analysis agents have no `Bash` tool: they keep `status.md` correct by keeping its inputs (their own documents) accurate, and the agents that do hold `Bash` regenerate the file itself. Authority model, one line per layer: **PM = Work Graph · Graphify = Code Graph · Orchestrator = Runtime** — plan.md owns what-work/order/dependency/owner, code intelligence owns source-code relationships, the orchestrator derives runtime readiness and dispatch; no layer implements another's job.
