...
describe("the shipped contracts", () => {
  it("exist for all nine agents, and not for `human` — a gate is not an agent", () => {
    expect(CONTRACTED_AGENTS).toHaveLength(9);
...
describe("assertContractsMatchRegistry", () => {
  it("throws with every problem listed when a contracts folder is wrong", () => {
    ...
    try {
      ...
      // eight missing files plus the tool mismatch
      expect((e as ContractRegistryMismatchError).problems.length).toBe(9);
    }
  });
