...
export interface OpenIssueRow {
  raw: string;
  /** Never null: a line only becomes a row once it names a role, so an ownerless row cannot exist. */
  owner: AgentStage;
  blocking: boolean;
...
/** Reads the rows of `## Open Issues — all phases`. A row counts only once it names a role — a header or separator names none. */
...
export function classifyQaFailure(reviewMd: string, opts: QaClassificationOptions = {}): StructuredFailure | null {
  ...
  const owners = [...new Set(deciding.map((r) => r.owner))];
  ...
  const category = CATEGORY_BY_OWNER[owner] ?? "unknown";
