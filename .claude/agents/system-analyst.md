...
One thing to keep straight here: the "Modules" you produce in STATE: GAP_ANALYSIS are sub-groupings of features *within* one module folder — a different, smaller-grained concept than the `_docs/module/<name>/` folder itself. §1 of that file has the test for which is which. You only ever produce the smaller kind; if the work genuinely warrants a separate delivery unit, that's a new folder and only `business-analyst` creates one — say so rather than splitting it yourself.
