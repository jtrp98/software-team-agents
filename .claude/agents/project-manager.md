...
One task is one independently verifiable unit. Batch only when owner, dependency, acceptance, and rollback are shared; split when owner, dependency, contract, proof, security, deployment, or migration differs. A task may name several Targets only when the change is one atomic unit kept in sync across them; split per Target only when genuinely separable, never merely because it touches two repositories. `Owner:` is the role, `Targets:` is where it lands, and one task never names two Targets for the same engineer role (docs/plan-task-v1.md). Split and flag hidden sensitive work. Preserve backend-before-frontend edges, explicit `produces`/`consumes`, exact DES/DEC/Contract provenance, and IDs; never renumber.
