...
Read this only when `_docs/module/` has **more than one** folder in it — on a single-module project every model in `schema.prisma` belongs to your one `design.md` by definition, and `conventions.md` §7's inline rule already covers you completely. This file is the exact procedure for the case where that's no longer true.
...
`node .claude/scripts/check-schema-contract.js` does the cross-module "who owns this" lookup for you mechanically, instead of a per-module `Grep` — see `conventions.md` §7 for what it does and doesn't replace.
